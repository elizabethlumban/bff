...
export const toPlainText = (htmlInput?: string | null) => {
  let html = htmlInput?.trim();
  if (isEmpty(htmlInput)) {
    return "";
  }
  if (html) {
    html = html
...
export const isEmpty = (htmlInput?: string | null) => {
...
export const isSubjectCode = (code?: string | null) => {
  const plainCode = code?.trim() || "";
...
export const isConcurrent = (rawPrefix: string, req: ReqType) => {
  ...
  return (
    prefix.match(/(can|may) be taken concurrently/gi) ||
...
const prefixMatchesReq = (rawPrefix: string, req: ReqType) => {
  const prefix = rawPrefix.toLowerCase().trim();
  if (!prefix) {
    return true;
  }
  ...
  // Do not match these ...
  if (
    prefix.match(/students: .* students:/gi) ||
  ...
  if (req === ReqType.PRE_REQ || req === ReqType.CO_REQ) {
    return (
      prefix.match(/can be taken concurrently/gi) ||
...
export const parsePretextWithAList = (htmlInput: string, reqType: ReqType): IParsedRule | null => {
  const html = htmlInput?.trim();
  const dom = new JSDOM(html);
  ...
  pre = pre.trim();
  codes = codes.trim();
  const subjectCodes = uniq((codes.match(/[A-Z]{4}\d{5}/g) || []).filter(Boolean));
  ...
  if (pre.match(/[A-Z]{4}\d{5}/) || subjectCodes.length === 0) {
    ...
    return {
      reqType,
      count: howManyAllowed(pre, subjectCodes.length, reqType),
      concurrent: !!isConcurrent(pre, reqType),
      codes: subjectCodes
    };
  } else {
    return null;
